...
keypoints:
- Subcomponent links can only be made with Items, whose status is one of the followings... In Fabrication (id = 100), Waiting on QA/QC Tests (id = 110), QA/QC Tests - Passed All (id = 120), or QA/QC Tests - Use As is (id = 140).
...
>|-------------+------------------|
>| [Updating Flags by Patching an Item(s)](#updating-flags-by-patching-an-items) | |
>|-------------+------------------|
...
- status (see [Updating Flags by Patching an Item(s)]({{ page.root }}/05-Data-Management-REST-API/index.html#updating-flags-by-patching-an-items) for more detail)
- Two shipping related flags (see [Shipping handoff process]({{ page.root }}/shippinghandoff/index.html) for more detail):
  - certified_qaqc
  - qaqc_uploaded
- is_installed
...
## Updating Flags by Patching an Item(s)

There are the following four flags that you can update by patching the corresponding item(s):
- Component Status flag (10 options with its initial value being set as "Unknown" (Status ID = 0))
- certified_qaqc
- qaqc_uploaded
- is_installed

Here,
`certified_qaqc` corresponds to **[Consortium Certified QA/QC flag]({{ page.root }}/shippinghandoff/index.html#the-two-key-flags)**
and `qaqc_uploaded` represents **[All QA/QC Test and Documentation Uploaded]({{ page.root }}/shippinghandoff/index.html#the-two-key-flags)**.

<br/><br/> 

First, let's determine values of these flags from an item:
There are two ways to check for this, one is by simply checking the item itself via the `/components/<eid>` API endpoint. The other is to do so directly through the `/components/<eid>/status` API end point. Let's take `Z00100400005-00001` as an example here.
~~~
...
~~~
{
  ...
  "link": {
    "href": "/cdbdev/api/v1/components/Z00100400005-00001/status",
    "rel": "self"
  },
  "status": "OK"
}
...
We can see values of those binary (true or false) flags, certified_qaqc, qaqc_uploaded, and is_installed.

We can also see the information on the Component Status flag with its ID = 170 and name = "Permanently Unavailable" in this example.
In general, a Component Status can be one of the following 10 options:

| Status ID | Status Name |
|----------|----------|
|   0 | Unknown (initial value) |
| 100 | In Fabrication |
...
Now let's update these flags using PID = D00599800009-00002 as an example:
~~~
CURL -H "Content-Type: application/json" -X PATCH -d @Patch_AnItem_2.json "${APIPATH}/components/D00599800009-00002" | jq
~~~
{: .language-bash}
where Patch_AnItem_2.json is;
~~~
{
    "part_id": "D00599800009-00002",
    "manufacturer": {
        "id": 7
    },
    "status": {
...
You can also PATCH the status of multiple items at once via the `/component-types/<type id>/bulk-update"` API endpoint. Let's update flag values of two items, `Z00100300001-06363` and `Z00100300001-06364` with this method.

~~~
CURL -H "Content-Type: application/json" -X PATCH -d @Patch_Items.json "${APIPATH}/component-types/Z00100300001/bulk-update" | jq
~~~
{: .language-bash}

And `Patch_Items.json` is this:
...
~~~
{
  "data": [
    {
      "link": {
        "href": "/cdbdev/api/v1/components/190431",
        "rel": "self"
      },
      "part_id": "Z00100300001-06363"
    },
    {
      "link": {
        "href": "/cdbdev/api/v1/components/190432",
        "rel": "self"
      },
      "part_id": "Z00100300001-06364"
    }
...
Adding subcomponents is a way to link existing items. Before attempting to link them, it is important to insure that the subcomponent type definitions are correct. The defining of subcomponent types can be done via [WEB-UI]({{ page.root }}/03-Setting-up-Types/index.html#connectors-an-optional-field) or [REST API]({{ page.root }}/03-Setting-up-Types/index.html#defining-a-component-type-with-the-rest-api).


> ## To be linked
>
> In order to link items, the component status of the intended subcomponent must be one of the following four statuses:
> - In Fabrication (id = 100)
> - Waiting on QA/QC Tests (id = 110)
> - QA/QC Tests - Passed All (id = 120)
> - QA/QC Tests - Use As is (id = 140)
{: .callout}
